 ...
 * Calculates daily calories, macros, meal distribution, and timeline projections.
 *
 * Safety constraints:
...
const MEAL_DISTRIBUTIONS = {
  2: [
    { name: 'Almuerzo', pct: 0.55 },
    { name: 'Cena', pct: 0.45 },
  ],
  3: [
    { name: 'Desayuno', pct: 0.30 },
    { name: 'Almuerzo', pct: 0.40 },
    { name: 'Cena', pct: 0.30 },
  ],
  4: [
  ...
  5: [
    { name: 'Desayuno', pct: 0.20 },
    { name: 'Snack AM', pct: 0.10 },
    { name: 'Almuerzo', pct: 0.30 },
    { name: 'Snack PM', pct: 0.10 },
    { name: 'Cena', pct: 0.30 },
  ],
  6: [
    { name: 'Desayuno', pct: 0.18 },
    { name: 'Snack AM', pct: 0.08 },
    { name: 'Almuerzo', pct: 0.25 },
    { name: 'Snack PM 1', pct: 0.10 },
    { name: 'Snack PM 2', pct: 0.10 },
    { name: 'Cena', pct: 0.29 },
  ],
};
 ...
 * @param {string} params.goal - 'lose_weight' | 'gain_muscle' | 'get_shredded'
 * @param {number} params.weight - Current weight in kg
 * @param {number|null} params.targetWeight - Target weight in kg
 * @param {number} params.mealsPerDay - Number of meals (2-6)
 * @param {string} params.deficitLevel - 'conservative' | 'moderate' | 'aggressive'
 */
export function generateNutritionPlan({ tdee, goal, weight, targetWeight, mealsPerDay, deficitLevel }) {
  ...
  // Recalculate protein calories after clamping
  const proteinCals = protein * 4;
  const remainingCals = dailyCalories - proteinCals;

  // Distribute remaining calories between carbs and fat using their original ratio proportion
  ...
  // Calculate actual macro ratios after clamping
  ...
  // 4. Timeline projection (always uses max 500 deficit for realistic pacing)
  ...
  if (targetWeight && targetWeight !== weight) {
    ...
    if (dailyChange > 0) {
      ...
      projectedChangeKg = goal === 'gain_muscle'
        ? deltaKg
        : -deltaKg;
    }
  }
